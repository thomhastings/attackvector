...
# XXX: WHOLE SCRIPT NEEDS TOTAL REBUILD/REDESIGN
...
# Well, this might as well have been a brainstorm. I still want a master git cronjob script
...
if [ ! -e "tails" ]
then
	git subtree add --prefix=tails git://git.immerda.ch/amnesia.git devel
...
do
	...
	then
		git subtree add --prefix=tails/kali/`echo $LINE | awk -F. '{print $1}'` git://git.kali.org/$LINE master
...
then
	...
	git svn clone http://armitage.googlecode.com/svn/trunk/ armitage
    ...
    if [ ! -e "tails/pentest/scan" ]
		...
		git subtree add --prefix=tails/pentest/scan/recon-ng https://bitbucket.org/LaNMaSteR53/recon-ng.git master
		...
		git subtree add --prefix=tails/pentest/scan/wpscan git://github.com/wpscanteam/wpscan.git master
		git commit -a -m "add extra subtree wpscan"
		# TODO: add codtitional statement like main TAILS, Kali for subtree pull vs subtree add
		git subtree add --prefix=tails/pentest/scan/dotdotpwn git://github.com/wireghoul/dotdotpwn.git master
		git commit -a -m "add extra subtree dotdotpwn"
		# TODO: add codtitional statement like main TAILS, Kali for subtree pull vs subtree add
		git subtree add --prefix=tails/pentest/scan/omphalos git://github.com/dankamongmen/omphalos.git master
	...
	if [ ! -e "tails/pentest/dos" ]
		...
		git subtree add --prefix=tails/pentest/ddos/schadcode git://github.com/emboss/schadcode.git master
		...
		mkdir -pv tails/pentest/misc
		git commit -a -m "add extra misc dir"
		git subtree add --prefix=tails/tilda git://github.com/tsloughter/tilda.git master # TODO add tilda config
		git commit -a -m "add extra subtree tilda"
		git subtree add --prefix=tails/pentest/misc/pentestgeek git://github.com/pentestgeek/scripts.git master
	...
	if [ ! -e "tails/pentest/mitm" ]
		...
		git subtree add --prefix=tails/pentest/mitm/FakeDns git://github.com/Crypt0s/FakeDns.git master
	...
	if [ ! -e "tails/pentest/scada" ]
		...
		git subtree add --prefix=tails/pentest/scada git://github.com/nxnrt/wincc_harvester.git master
	...
	if [ ! -e "tails/pentest/shells" ]
		...
		git subtree add --prefix=tails/pentest/shells/webhandler git://github.com/lnxg33k/webhandler.git master
	...
	if [ ! -e "tails/pentest/webapp" ]
		...
		git subtree add --prefix=tails/pentest/webapp/owtf git://github.com/7a/owtf.git master
		git commit -a -m "add extra subtree owtf"
		# TODO: add codtitional statement like main TAILS, Kali for subtree pull vs subtree add
		git subtree add --prefix=tails/pentest/webapp/pgconsole git://github.com/pentestgeek/pgconsole.git master
	...
	if [ ! -e "tails/pentest/wifi" ]
		...
		git subtree add --prefix=tails/pentest/wifi/lorcon https://code.google.com/p/lorcon master
